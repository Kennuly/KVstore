...
#else
		struct Conn_item item = {0};   // 这里必须加 struct
		ret = recv(fd, item.rbuffer_, BUFFER_LEN, 0);
		if (ret > 0)
		{
			if (fd > MAX_CLIENT_NUM)
				printf("read from server: %.*s\n", ret, item.rbuffer_);
			KVstore_request(&item);
			item.wlen_ = strlen(item.wbuffer_);
			ret = send(fd, item.wbuffer_, item.wlen_, 0);
			if (ret == -1)
			{
				close(fd);
				break;
